...
def input_to_points(input)
  start = [0, 0]
  input.map do |code|
    ret = Movement.new(start, code).move
    start = ret.last
    ret
  end.flatten(1)
end

points1 = input_to_points(INPUT1)
points2 = input_to_points(INPUT2)
intersection = points1 & points2

print "First problem: #{intersection.map {|point| point[0].abs + point[1].abs }.min}\n"
print "Second problem: #{intersection.map {|point| points1.index(point) + points2.index(point) + 2}.min}\n"
