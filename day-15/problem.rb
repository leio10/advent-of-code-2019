...
while tracks.any?
  ...
  [[1, 2], [2, 1], [3, 4], [4, 3]].each do |direction, back|
    new_pos = move(direction, pos)
    next if map[new_pos]

    result = robot.run([direction])
...
while tracks.any?
  ...
  [[0, -1], [0, 1], [-1, 0], [1, 0]].each do |direction|
    new_pos = [pos.first + direction.first, pos.last + direction.last]

    next unless map[new_pos] == '·'

    second_problem = track.size if track.size > second_problem

    map[new_pos] = 'O'
    tracks << track + [new_pos]
  end

  show(map)
end

print "First problem: #{first_problem}\n"
print "Second problem: #{second_problem}\n"
